...
export async function getIncomeBreakdown(from?: string, to?: string) {
  ...
  // ---- DaaS rental engagements ----------------------------------------------
  // Finds every submitted Sales Order that carries a DaaS line (upfront or rental),
  // regardless of delivery status, and folds the upfront + rental SOs that share a
  // quotation (custom_reference_no) into one engagement via detectDaaSEngagements.
  // Returns the engagements plus the set of SO names that belong to a DaaS deal, so
  // getOutstandingOrders can keep them out of the goods-delivery orders list.
  export async function getRentalEngagements() {
    return cached('rental-engagements', 5 * 60_000, async () => {
      // 1) Parent SO names that have a DaaS line (via the Sales Order Item child table).
      const daasRows = await frappeGet(`/api/resource/${encodeURIComponent('Sales Order Item')}`, {
        parent: 'Sales Order',
        filters: [['item_code', 'in', DAAS_ITEM_CODES], ['docstatus', '=', 1]],
        fields: ['parent'],
        limit_page_length: 0,
      });
      const soNames: string[] = [...new Set(((daasRows ?? []) as any[]).map((r) => String(r.parent)))];
      if (!soNames.length) {
        return { engagements: [] as ReturnType<typeof detectDaaSEngagements>, soNames: new Set<string>() };
      }

      // 2) Parent-level fields for those SOs.
      const parents = await getList('Sales Order', {
        filters: [['name', 'in', soNames]],
        fields: ['name', 'customer', 'customer_name', 'custom_reference_no',
                 'transaction_date', 'delivery_date', 'status', 'per_billed',
                 'per_delivered', 'base_grand_total', 'base_rounded_total'],
      });

      // 3) Their line items (child table), batched.
      const itemsByParent: Record<string, any[]> = {};
      for (const chunk of chunked(soNames, 50)) {
        const its = await frappeGet(`/api/resource/${encodeURIComponent('Sales Order Item')}`, {
          parent: 'Sales Order',
          filters: [['parent', 'in', chunk]],
          fields: ['parent', 'item_code', 'item_name', 'item_group', 'gst_hsn_code',
                   'qty', 'rate', 'amount', 'cgst_rate', 'sgst_rate', 'igst_rate'],
          limit_page_length: 0,
        });
        for (const it of its ?? []) (itemsByParent[it.parent] ??= []).push(it);
      }

      // 4) Assemble into the SalesOrder shape the model expects, then detect.
      const sos = (parents ?? []).map((p: any) => ({
        name: p.name,
        customer_name: p.customer_name || p.customer,
        custom_reference_no: p.custom_reference_no ?? null,
        transaction_date: p.transaction_date,
        delivery_date: p.delivery_date,
        status: p.status,
        per_billed: Number(p.per_billed || 0),
        per_delivered: Number(p.per_delivered || 0),
        base_grand_total: Number(p.base_grand_total || 0),
        base_rounded_total: Number(p.base_rounded_total || 0),
  ...
  export async function getOutstandingOrders(limit = 50) {
    return cached(`outstanding-orders:${limit}`, 5 * 60_000, async () => {
      ...
      const awaitingInstallationValue = engagements
        .filter((e) => e.upfrontStatus === 'billed'
          && e.upfrontOrders.some((o) => (o.per_delivered ?? 0) < 100))
        .reduce((s, e) => s + e.upfrontGross, 0);
      ...
      const upcomingDeliveries = orders
        .filter((o: any) => o.deliveryDate && !o.overdue)
        .sort((a: any, b: any) => (a.deliveryDate < b.deliveryDate ? -1 : 1));
      const nextDelivery = upcomingDeliveries[0] ?? null;
      const overdueCount = orders.filter((o: any) => o.overdue).length;
