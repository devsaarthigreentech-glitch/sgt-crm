...
export default function PartnerPortal({ onExit }: { onExit?: () => void } = {}) {
  ...
  const tabs = tabsForPartner(me)
  const active = tabs.includes(tab) ? tab : tabs[0]
  const SHELL = 760   // centered content column on desktop; fills the screen on mobile

  return (
    <div style={{ backgroundColor: t.ground, flex: 1, width: '100%', height: '100vh', overflowY: 'auto' }}>
      {/* identity bar — full-bleed green, centered content */}
      <div style={{ backgroundColor: t.green, color: '#fff', padding: '16px 20px' }}>
        <div style={{
          maxWidth: SHELL, margin: '0 auto', width: '100%',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8,
        }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700 }}>{me.name}</div>
      ...
      {/* tabs — full-bleed divider, centered row */}
      <div style={{ borderBottom: `1px solid ${t.border}`, backgroundColor: t.surface }}>
        <div style={{
      ...
      <div style={{ maxWidth: SHELL, margin: '0 auto', padding: 20 }} key={`${active}-${reloadKey}`}>
        {active === 'leads' && <MyLeads />}
        {active === 'register' && <RegisterLead onDone={() => { setReloadKey(k => k + 1); setTab('leads') }} />}
        {active === 'customers' && <MyCustomers />}
        {active === 'scorecard' && <Scorecard />}
