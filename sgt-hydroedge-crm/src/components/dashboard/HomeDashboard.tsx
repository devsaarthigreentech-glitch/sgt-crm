...
function DirectorDashboard({ leads, activeLeads, isMobile, role, onLeadClick, navigate }: {
  ...
  // Total SLA issues = pipeline breaches + risks + overdue order fulfilments.
  const slaTotal = slaBreaches.length + slaRisk.length + overdueOrders
  const slaSubParts = [
    `${slaBreaches.length} stage breach`,
    `${slaRisk.length} at risk`,
  ]
  if (overdueOrders > 0) slaSubParts.push(`${overdueOrders} order overdue`)
    ...
    <div style={{ maxWidth: 900, display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{
        ...
        <StatCard
          ...
          sub={slaSubParts.join(' · ')}
          accent={slaBreaches.length > 0 || overdueOrders > 0 ? '#A02B1F' : '#A86A18'}
...
// ─────────────────────────────────────────────
// Supply Chain Dashboard — capacity calculator only
// ─────────────────────────────────────────────

function SupplyChainDashboard() {
  return (
    <div style={{ maxWidth: 900, display: 'flex', flexDirection: 'column', gap: 24 }}>
      <ErpInsights mode="capacity" />
    </div>
  )
}
