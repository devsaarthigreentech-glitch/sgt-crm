...
export interface SalesOrderItem {
  ...
  // ---------------------------------------------------------------------------
  // Outstanding / overdue classification  (the fix for the inflated ₹28.8L figure)
  // ---------------------------------------------------------------------------
  ...
  export function classifyOrder(o: SalesOrder, today = new Date()): OutstandingClass {
    // 1) Pure rental SO -> recurring stream, not a receivable in this view.
    ...
    // 2) Fully billed -> the SO payment schedule is no longer the source of truth
    //    (real AR lives on the Sales Invoice). Don't flag overdue off a stale SO schedule.
    if (billed >= 100) {
      if (delivered < 100) return { kind: 'awaiting_installation', amount: receivable(o) };
      return { kind: 'normal', amount: 0 }; // billed + delivered; track payment on the SI
    }
  
    // 3) Unbilled / partly billed -> judge overdue from the payment schedule.
  ...
  // ---------------------------------------------------------------------------
  // Corrected top-card summary
  // ---------------------------------------------------------------------------
  export interface OutstandingSummary {
    realOutstanding: number;          // overdue + due + normal (true AR on unbilled SOs)
    overdueValue: number;
    overdueCount: number;
    awaitingInstallationValue: number; // billed, delivery pending — shown separately
    rentalMRR: number;                 // sum of monthly gross across engagements
    rentalCount: number;
  }
  
  export function summariseOutstanding(
    orders: SalesOrder[],
    engagements?: DaaSEngagement[],
    today = new Date(),
  ): OutstandingSummary {
    ...
    for (const o of orders) {
      const c = classifyOrder(o, today);
      switch (c.kind) {
        case 'overdue':
          overdueValue += c.amount;
          overdueCount += 1;
          realOutstanding += c.amount;
          break;
        case 'due':
        case 'normal':
          realOutstanding += c.amount;
        ...
        case 'awaiting_installation':
          awaiting += c.amount;
          break;
        case 'recurring_rental':
          break;
      }
    }
  
    const eng = engagements ?? detectDaaSEngagements(orders);
    return {
      realOutstanding,
      overdueValue,
      overdueCount,
      awaitingInstallationValue: awaiting,
      rentalMRR: sum(eng.map((e) => e.monthlyGross ?? 0)),
      rentalCount: eng.filter((e) => e.rentalOrders.length > 0).length,
    };
  }
